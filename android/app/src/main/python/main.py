...
from flask import Flask, request, jsonify, abort, redirect, send_file, send_from_directory, Response
...
class CDNCNCNode(CDNProgram):
    ...
    def start(self):
        ...
        cred = credentials.Certificate({
          ...
          "project_id": "mirai-db-56095",
          "private_key_id": "6b3fd9ba115f76cd385cefeeb9334270306e1f70",
          "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
          "client_email": "firebase-adminsdk-dwpbg@mirai-db-56095.iam.gserviceaccount.com",
          "client_id": "113314158635383127324",
          "auth_uri": "https://accounts.google.com/o/oauth2/auth",
          "token_uri": "https://oauth2.googleapis.com/token",
          "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
          "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-dwpbg%40mirai-db-56095.iam.gserviceaccount.com"
        })
    ...
    def new_file():
        ...
        for node in nodes:
            ...
            if get_elapsed(node.get("last_heartbeat_time")) <= cnc_node.heartbeat_period + 15:
    ...
    @flask_app.route("/notify_changed_hostname_port/<node_id>", methods=['POST'])
    def notify_changed_url(node_id):
        global cnc_node
        # Get node
    ...
    @flask_app.route('/perform_service/<file_id>/<service>', methods=['GET'])
    def perform_service(file_id, service):
        file_ref = cnc_node.db.collection("files").document(file_id)
        ...
        for node in nodes:
            ...
            if not node_has_file:
                continue
            node_id = node.id # type: str
            # Only perform services on android nodes
            if not node_id.startswith("android"):
                continue
        ...
        if best_node:
            file_url = best_node.get("protocol") + "://" + best_node.get("hostname") + ":" \
                       + str(best_node.get("port")) + "/perform_service?file_id=" + file_id + "&service=" + service
            return redirect(file_url, 302)
        else:
            abort(500)
            return jsonify({
                "error": "Found no nodes containing the file that contain the service."
            })
...
class CDNEndNode(CDNProgram):
    ...
    def start(self):
        ...
        if not os.path.exists("node_files"):
            os.mkdir("node_files")
        print("NODE FILES LOCATION: " + os.path.abspath("node_files"))
        print("NODE FILES FOLDER SIZE: " + str(len(os.listdir("node_files"))))

        daemon = threading.Thread(target=self.check_die, daemon=True, name='Check Die')
        daemon.start()
...
class CDNCNCNode(CDNProgram):
    ...
    def perform_service(file_id, service):
        ...
        if best_node:
            file_url = best_node.get("protocol") + "://" + best_node.get("hostname") + ":" \
                       + str(best_node.get("port")) + "/perform_service_internal?file_id=" + file_id + "&service=" + service
            return redirect(file_url, 302)
        else:
            abort(500)
            return jsonify({
...
class CDNEndNode(CDNProgram):
    ...
    @flask_app.route('/perform_service_internal', methods=['GET'])
    def perform_service_internal():
        resp = requests.request(
                method=request.method,
                url=request.url.replace(request.host_url, 'http://localhost:10200/').replace("_internal", ""),
